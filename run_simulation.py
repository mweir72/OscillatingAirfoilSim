...
def run_single():
    ...
    plt.grid()
    plt.subplot(6, 1, 5)
    plt.plot(t, res["theta_deg"])
    plt.title("Pitch (deg)")
    plt.grid()
    plt.subplot(6, 1, 6)
    plt.plot(t, res["x_pos"])
    plt.title("Stroke Pos")
    plt.grid()
...
def run_sweep(label, values, param_key):
    ...
    for v in values:
        ...
        plt.subplot(3, 2, 1)
        plt.plot(t, r["L"], label=lab)
        plt.title("Lift")
        plt.grid()
        plt.subplot(3, 2, 2)
        plt.plot(t, r["D"], label=lab)
        plt.title("Drag")
        plt.grid()
        plt.subplot(3, 2, 3)
        plt.plot(t, r["P"], label=lab)
        plt.title("Power")
        plt.grid()
        plt.subplot(3, 2, 4)
        plt.plot(t, r["eta"], label=lab)
        plt.title("Efficiency")
        plt.grid()
        plt.subplot(3, 2, 5)
        plt.plot(t, r["theta_deg"], label=lab)
        plt.title("Pitch")
        plt.grid()
        plt.subplot(3, 2, 6)
        plt.plot(t, r["x_pos"], label=lab)
        plt.title("Stroke Pos")
        plt.grid()
